...
- Grafana core repo checked out at `../grafana` on the branch above
...
[plugins]
allow_loading_unsigned_plugins = canvas

[log]
level = debug
filters = plugin.canvas:debug
```
...
```bash
curl -s -u admin:admin http://localhost:3000/api/plugins/canvas/settings | jq '.module'
# should return: "public/plugins/canvas/module.js"
# (not "core:plugin/canvas")
```
