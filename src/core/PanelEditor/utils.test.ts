import { type FieldConfig, type FieldConfigSource, type PanelPlugin } from '@grafana/data';
...
describe('updateDefaultFieldConfigValue', () => {
  ...
  `(
    ...
    ({
      ...
      expect(updateDefaultFieldConfigValue(cfg as FieldConfigSource, property, newValue, isCustom).defaults).toEqual(
        expected
      );
    }
  );
